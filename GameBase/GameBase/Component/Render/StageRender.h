...
public:
	StageRender();
	virtual ~StageRender();
	static constexpr ComponentID id_{ ComponentID::StageRender };

	ComponentID GetID(void) const override
	{
		return id_;
	}
	void Draw(int shadowMap = -1, int buff = -1, int cubeTex = -1) override;
...
private:
	...
	void DrawShader(int shadowMap = -1, int buff = -1,int cubeTex = -1);
	...
	/// <param name="shadowMap"></param>
	/// <param name="buff"></param>
	void DrawNonShader(int shadowMap = -1, int buff = -1,int cubeTex = -1);
	...
	void (StageRender::* draw_)(int, int,int);
	void (StageRender::* drawDepth_)(int, int);
	// 定数バッファ
	int cbuff_;
	float* writer_;
	int cbuffFloat4_;
